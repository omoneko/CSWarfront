...
NAME_MAP = {
    ...
    # The 3 models the user added to models.blend on 2026-07-30 (the last three that had
    # been standing in on auto-generated models until then)
    'spaag': 'Unit_AntiAir',
    'loiteringdrone': 'Unit_SuicideDrone',
    'basemissile': 'Building_MissileBase',
    # Task87: the bomber's dropped bomb (a small prop for BombFx). As of 2026-07-31
    # models.blend has no bomb object yet; an interim model (generated by the scratchpad's
    # gen_bomb_obj.py) is in use. If the user adds one under a name like "17_Bomb", this
    # mapping overwrites the interim model on the next export (see OPTIONAL_MODELS =
    # its absence is not an error).
    'bomb': 'Prop_Bomb',
    'bomb500lb': 'Prop_Bomb',  # the actual name the user added 2026-07-31 (17_Bomb_500lb)
    # Task90: the ballistic missile (MissileVisuals) and the AA/interceptor missile
    # (AaMissileFx). Also absent from models.blend as of 2026-07-31 (interim models reuse
    # MissileDisaster's OBJs). Adding them as "18_Ballistic_Missile"/"19_Interceptor" etc.
    # overwrites them on the next export.
    'ballisticmissile': 'Prop_BallisticMissile',
    'interceptor': 'Prop_Interceptor',
    'interceptormissile': 'Prop_Interceptor',
    # Task99: the supply truck (20_Supply_Truck, user-added 2026-08-03). Replaces the
    # APC-model stand-in used so far (UnitMeshSource's fallback) with a dedicated model.
    'supplytruck': 'Unit_SupplyTruck',
    # Task101 (Update 3, the 8 models the user added 2026-08-04): 4 fortifications +
    # 2 helicopters + 2 rail. 24_Trench had its model replaced, so always re-export it.
    'bunker': 'Fort_Bunker',
    ...
    # 2026-08-05: the military freight train was split into one independent object per
...
# NAME_MAP entries that are not an error when the object doesn't exist in the .blend yet
# (when present they export normally and overwrite the interim model).
...
# 2026-07-30: emptied out, since all 16 models are now covered by Blender models.
